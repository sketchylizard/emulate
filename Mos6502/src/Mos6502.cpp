...
struct Operations
{
  ...
  {
    assert(cpu.HasFlag(Mos6502::Binary) == false);  // Decimal mode not supported
    ...
    cpu.SetFlag(Mos6502::Carry, (sum & 0x100) != 0);
    cpu.SetFlag(Mos6502::Overflow, ((~(a ^ m) & (a ^ result)) & 0x80) != 0);
    cpu.SetZN(result);
